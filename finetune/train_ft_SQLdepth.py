    ...
    for epoch in range(args.epoch, epochs):
        ...
        # if is_rank_zero( args) else enumerate(train_loader):
            ...
            if should_write and step % args.validate_every == 0:
                ...
                if metrics.get('abs_rel') is not None and metrics['abs_rel'] < best_loss and should_write:
...
def validate(args, model, test_loader, criterion_ueff, epoch, epochs, device='cpu'):
    with torch.no_grad():
        ...
        # if is_rank_zero( args) else test_loader:
            ...
            pred_med = np.median(pred)
            gt_med = np.median(gt_depth)
            if np.isnan(gt_med) or np.isnan(pred_med) or pred_med == 0:
                continue

            ratio = gt_med / pred_med
            ...
            sample_metrics = utils.compute_errors(gt_depth, pred)
